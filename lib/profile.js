...
const fetchUserProfile = async (dispatch) => {
    ...
    const userData = await fetcher(
        `${process.env.NEXT_PUBLIC_STRAPI_URL}/users/28?populate[0]=information_gathering&populate[1]=favorite_activity.favourite_activities&populate[2]=profile_image&populate[3]=user_day_plan`,
        {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
